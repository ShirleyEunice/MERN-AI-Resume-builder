...
const ResumeBuilder = () => {
  ...
  const changeResumeVisibility = async() =>{
     try{
      const formData = new FormData(); // web API which is easy to send files and data together to a server
      ...
      console.log(error.message || "Error changing resume visibility: ", error);
    }
  }

  const handleShare = ()=>{
    const frontendUrl = window.location.href.split('/app/')[0];
    const reusumeUrl = frontendUrl + '/view/' + resumeId;

    if(navigator.share){
      navigator.share({url: reusumeUrl, text: "My Resume", })
    }else{
  ...
  const saveResume = async()=>{
    setLoading(true);
    try {
      let updatedResumeData = structuredClone(resumeData); // seperate copy of the resumeData, not just another reference to the same object
      ...
      setLoading(false);
      console.log(error.message || "Error saving resume: ", error);
    }
  }
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          ...
          <div className="relative lg:col-span-5 rounded-md overflow-hidden">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1">
              ...
              <hr
                className="absolute top-0 left-0 h-1 bg-linear-to-r from-green-500 to-green-600 border-0 transition-all duration-1000"
                style={{
                  width: `${
              ...
              <div className="flex justify-between items-center mb-6 border-b border-gray-300 py-1">
                ...
                <div className="flex items-center">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-3 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all ${
